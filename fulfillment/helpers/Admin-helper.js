...
function create_cards(body, session_to_use) {
    ...
    for (i = 0; i < crew.length; i++) {
        ...
        var card = new builder.HeroCard(session_to_use)
            ...
            .buttons([builder.CardAction.imBack(session_to_use, 'Hotel Details for' + body[i].UserID,'select hotel details for' + body[i].UserID),
            builder.CardAction.imBack(session_to_use, 'Flight Details for ' + body[i].UserID,'select flight details for' + body[i].UserID)])
        cards.push(card);
    }
